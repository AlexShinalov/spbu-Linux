...
void sys::bridge_interface::init() {
    ...
    s.call(fildes::operation::bridge_add, this->_name[0]);
}

void sys::bridge_interface::destroy() {
    socket s{socket_address_family::inet,socket_type::datagram};
    s.call(fildes::operation::bridge_del, this->_name[0]);
}
...
void sys::bridge_interface::add(const sys::network_interface& iface) {
    ...
    socket s{socket_address_family::inet,socket_type::datagram};
    s.call(fildes::operation::bridge_add_interface, req);
}
...
void sys::bridge_interface::remove(const sys::network_interface& iface) {
    ...
    req.ifr_ifindex = iface.index();
    socket s{socket_address_family::inet,socket_type::datagram};
    s.call(fildes::operation::bridge_del_interface, req);
}
